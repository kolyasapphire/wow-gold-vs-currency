...
type NamesRes = Record<string, string>;

export const getData = async () => {
	"use cache";
	cacheLife({ expire: 24 * 60 * 60 });

	const { BLIZZARD_KEY, BLIZZARD_SECRET } = process.env;

	if (!BLIZZARD_KEY || !BLIZZARD_SECRET) throw new Error("Bad env");
	...
	return { data: sliced, totalItems: withGold.length, goldPosition };
};
